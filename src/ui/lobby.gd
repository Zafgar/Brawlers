...
func _handle_join_input(device: int, edge: Dictionary) -> void:
	...
	if edge.accept:
		if entry.is_empty():
			...
			AudioMgr.play("ui_lock" if entry.ready else "ui_back")
			_check_all_ready()
		return
...
func _tick_bot_picks(delta: float) -> void:
	...
	_bot_pick(next_bot)
	AudioMgr.play("ui_lock", 0.03, -7.0)
...
func _draw_join() -> void:
	...
	for team in [0, 1]:
		...
		UiKit.draw_text(self, Vector2(panel_x + 710, 254),
			"%d paikallista" % _team_human_count(team), 18,
	...
	UiKit.draw_text(self, Vector2(960, 1030),
		"Ottelu on aina 4v4 — paikallisia pelaajia voi olla yhteensä 1–4", 18,
...
func _draw_heroes() -> void:
	_draw_title("VALITSE MOBA-SANKARI", 72)
	UiKit.draw_text(self, Vector2(960, 145),
		"Roolijako: 1 TOP  •  1 JUNGLE  •  2 BOTTOM — AI täydentää puuttuvat roolit",
		17, Palette.GOLD, true)
...
func _draw_hero_tile(rect: Rect2, tile_index: int) -> void:
	...
	var med := rect.position + Vector2(rect.size.x / 2.0, 38.0)
	draw_arc(med, 31.0, _time, _time + TAU * 0.8, 24, Palette.with_alpha(Palette.glow(c1, 1.3), 0.5), 2.0)
	draw_circle(med, 28.0, Palette.darker(c2, 0.55))
	draw_circle(med, 25.0, c1)
	draw_circle(med + Vector2(0, 9), 18.0, Palette.with_alpha(c2, 0.35))
	HeroIcon.draw_symbol(self, hero_id, med, 16.0)

	UiKit.draw_text(self, rect.position + Vector2(rect.size.x / 2.0, 80.0),
		def["name"], 20, Palette.TEXT_MAIN, true, 3)
	var stars := ""
	for i in range(3):
		stars += "★" if i < int(def["difficulty"]) else "☆"
	UiKit.draw_text(self, rect.position + Vector2(rect.size.x / 2.0, 106.0),
		"%s  %s" % [def["role"], stars], 13, c1, true)
	UiKit.draw_text(self, rect.position + Vector2(rect.size.x / 2.0, 128.0),
		def["weapon"], 10, Palette.TEXT_DIM, true)
