...
##                 sankarikoosteet.
##
## Tasapelit näytetään erikseen. Voittoprosentit lasketaan ratkaistuista peleistä,
## ja sankarin vahinko/min hänen omasta yhteenlasketusta peliajastaan.
...
static func build(snapshots: Array, intro: Array) -> String:
	...
	for mi in range(snapshots.size()):
		...
		if winner >= 0 and winner <= 1:
			...
			draws += 1
		var elapsed: float = float(r["elapsed"])
		total_time += elapsed
		lines.append("--- OTTELU %d ---" % (mi + 1))
		lines.append("Kesto: %s | Voittaja: %s (%s)" % [
			_fmt(elapsed), _team(winner), str(r["reason"])])
	...
	lines.append("Voitot: Sininen %d - %d Oranssi | Tasapelit/timeoutit: %d" % [
		wins[0], wins[1], draws])
	lines.append("Keskimääräinen kesto: %s   (* = ihmispelaaja)" % _fmt(total_time / float(count)))
	lines.append("")
	_hero_table(lines, agg)
...
static func build_sweep(results: Array, intro: Array) -> String:
	...
	for e in results:
		...
		var drawn: bool = winner < 0 or winner > 1
		if drawn:
			draws += 1
		total_time += elapsed
		var dmg0 := 0.0
		...
		for h in snap["heroes"]:
			if int(h["team"]) == 0:
				...
				dmg1 += float(h["damage"])
		_arch_add(arch, str(e["ba"]), winner == 0, drawn, elapsed, dmg0)
		_arch_add(arch, str(e["oa"]), winner == 1, drawn, elapsed, dmg1)
		for h in snap["heroes"]:
			_accumulate(agg, h, winner, elapsed)

	lines.append("=== KOKOONPANOTYYPIT (%d ottelua; tasapelit/timeoutit %d) ===" % [
		results.size(), draws])
	lines.append("  tyyppi       | pelit | V-T-H      | voitto%* | keskikesto | vah/peli")
	var arows: Array = arch.values()
	for a in arows:
		var decided: float = maxf(float(a["games"] - a["draws"]), 0.0)
		a["wr"] = float(a["wins"]) / decided if decided > 0.0 else 0.0
	arows.sort_custom(func(x, y): return float(x["wr"]) > float(y["wr"]))
	for a in arows:
		var g: float = maxf(float(a["games"]), 1.0)
		var losses: int = int(a["games"]) - int(a["wins"]) - int(a["draws"])
		lines.append("  %-12s | %3d   | %2d-%2d-%2d   | %6.1f %% | %s      | %6d" % [
			str(a["name"]), int(a["games"]), int(a["wins"]), int(a["draws"]), losses,
			float(a["wr"]) * 100.0, _fmt(float(a["time"]) / g),
			int(float(a["damage"]) / g)])
	lines.append("  * voitto%% lasketaan vain ratkaistuista otteluista.")

	lines.append("")
	lines.append("=== ANOMALIAT (mahdollisesti rikki / epätasapaino) ===")
	var flagged := false
	for a in arows:
		var decided: int = int(a["games"]) - int(a["draws"])
		if decided >= 4 and (float(a["wr"]) >= 0.75 or float(a["wr"]) <= 0.25):
			flagged = true
			lines.append("  Kokoonpano '%s': voitto%% %.0f (%d ratkaistua, %d tasapeliä) — %s" % [
				str(a["name"]), float(a["wr"]) * 100.0, decided, int(a["draws"]),
				("liian vahva?" if float(a["wr"]) >= 0.75 else "liian heikko?")])

	var mean_dpm := _mean_dpm(agg)
	for id in agg:
		var a: Dictionary = agg[id]
		if int(a["games"]) < 3:
			continue
		var dpm: float = _dpm(a)
		if float(a["damage"]) <= 1.0:
			...
			var util: float = (float(a["healing"]) + float(a["mitigated"])) \
				/ maxf(float(a["games"]), 1.0)
...
static func _hero_table(lines: Array, agg: Dictionary) -> void:
	lines.append("Sankariteho (järjestetty vahinko/min; voitto% ratkaistuista peleistä):")
	lines.append("  sankari  | pel | tas | K / D / A   |vah/min|torni |viidak|paran |vaim. | CS  |voit%")
	...
	for a in rows:
		var g: float = maxf(float(a["games"]), 1.0)
		var decided: float = maxf(float(a["games"] - a["draws"]), 1.0)
		lines.append("  %-8s | %2d  | %2d  | %4.1f/%4.1f/%4.1f | %5d |%5d |%5d |%5d |%5d |%4.1f |%3d%%" % [
			str(a["hero_id"]), int(a["games"]), int(a["draws"]),
			...
			int(round(100.0 * float(a["wins"]) / decided))])


## Vahingon lähteet + selviytyminen.
static func _survivability_table(lines: Array, agg: Dictionary) -> void:
	...
	for a in rows:
		...
		if int(a["games"]) >= 3 and taken > 1.0:
			if pct_t >= 22.0:
				flags.append("  '%s': %.0f%% vahingosta TORNEILTA — dive-turva/positiointi?" % [
					str(a["hero_id"]), pct_t])
			if pct_n >= 18.0:
				flags.append("  '%s': %.0f%% vahingosta VIIDAKOSTA — varoo mobeja huonosti?" % [
					str(a["hero_id"]), pct_n])
	...
	for id in ids:
		...
		if ult_vpc >= 0.0 and ult_casts >= 2.0 and not abil_vpc.is_empty():
			...
			if abil_mean > 0.0 and ult_vpc < abil_mean:
				ult_flags.append("  '%s': ult arvo/käyttö %d < a1/a2-kykyjen ka %d — käyttö tai teho tarkistettava" % [
					id, int(ult_vpc), int(abil_mean)])
	lines.append("")
	lines.append("  -- rooliprofiili (mihin mitattu panos meni) --")
	for p in profiles:
		lines.append(str(p))
	if not ult_flags.is_empty():
		lines.append("")
		lines.append("  -- ultit joiden arvo/käyttö jää a1/a2-kykyjen alle --")
		for f in ult_flags:
			lines.append(str(f))
	lines.append("")
	if suspects.is_empty():
		lines.append("  Kaikki käytetyt kyvyt tuottivat mitattavaa taisteluarvoa.")
	else:
		lines.append("  * = käytetty >=3 kertaa mutta EI mitattavaa taisteluarvoa.")
		lines.append("      Kyky voi olla rikki TAI tarkoituksella puhdas liikkumis-/asemointikyky: " \
			+ ", ".join(PackedStringArray(suspects)))


static func _dpm(a: Dictionary) -> float:
	var minutes: float = float(a.get("elapsed", 0.0)) / 60.0
	return float(a["damage"]) / maxf(minutes, 0.1)


static func _mean_dpm(agg: Dictionary) -> float:
	var total := 0.0
	var n := 0
	for id in agg:
		var a: Dictionary = agg[id]
...
static func _arch_add(arch: Dictionary, name: String, won: bool, drawn: bool,
		elapsed: float, damage := 0.0) -> void:
	if not arch.has(name):
		arch[name] = {"name": name, "games": 0, "wins": 0, "draws": 0,
			"time": 0.0, "damage": 0.0}
...
static func _accumulate(agg: Dictionary, h: Dictionary, winner: int, elapsed: float) -> void:
	var id: String = str(h["hero_id"])
	if not agg.has(id):
		agg[id] = {"hero_id": id, "games": 0, "draws": 0, "elapsed": 0.0,
			"kos": 0.0, "deaths": 0.0, "assists": 0.0, "damage": 0.0,
			"taken": 0.0, "structure_damage": 0.0, "jungle_damage": 0.0,
			"mitigated": 0.0, "minion_kills": 0.0, "healing": 0.0, "wins": 0,
		...
		for sname in h["slots"]:
			...
			if not asl.has(sname):
				asl[sname] = {"casts": 0.0, "hits": 0.0, "damage": 0.0,
					"heal": 0.0, "stun": 0.0, "slow": 0.0, "root": 0.0,
					"kb": 0.0, "shield": 0.0, "buff": 0.0}
			var dst: Dictionary = asl[sname]
			for key in ["casts", "hits", "damage", "heal", "stun", "slow", "root",
					"kb", "shield", "buff"]:
				dst[key] += float(sr.get(key, 0))
	if winner < 0 or winner > 1:
		a["draws"] += 1
	elif int(h["team"]) == winner:
		a["wins"] += 1
