...
func _ultimate(_dir: Vector2) -> void:
	...
	AudioMgr.play("blessing", 0.05, -3.0)
	arena.shake(0.4)
	Fx.flash(arena, global_position, Palette.glow(Color("c7a8ff"), 1.6), ULT_RADIUS * 0.6, 0.5)
	Fx.vortex(arena, global_position, Palette.glow(hero_color(), 1.4), ULT_RADIUS, 1.1)
	Fx.ring(arena, global_position, Palette.glow(hero_color(), 1.5), ULT_RADIUS, 0.7, 8.0)
	for enemy in arena.heroes_in_circle(global_position, ULT_RADIUS, 1 - team, true, true):
