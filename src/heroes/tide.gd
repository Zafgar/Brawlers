...
func _ultimate(dir: Vector2) -> void:
	var landing := aimed_ult_ground_position(dir)
	landing = arena.map.clamp_to_field(landing, radius)
	var d: Vector2 = (landing - global_position).normalized()
	var rush_dur := maxf(global_position.distance_to(landing) / RUSH_SPEED, 0.12)
	arena.popup(global_position + Vector2(0, -84), "VESIPATSAS!", Palette.glow(Color("4ad4ff"), 1.6), 26)
	AudioMgr.play("tide_ult", 0.05, -2.0, landing)
	arena.shake(0.35)
	Fx.ultimate_warning(arena, landing, Color("4ad4ff"), Palette.team(team),
		RUSH_RADIUS, rush_dur, "tide")
...
func _rush_slam(rush_dur: float) -> void:
