...
func _ultimate(_dir: Vector2) -> void:
	...
	AudioMgr.play("bramble_ult", 0.05, -3.0, center)
	Fx.ultimate_warning(arena, center, Color("7ed957"), Palette.team(team), 300.0, 0.38, "bramble")
	_grow_garden(center)


func _grow_garden(center: Vector2) -> void:
	await get_tree().create_timer(0.38).timeout
	if not is_inside_tree():
		return
	_act("ult")
	arena.shake(0.5)
	AudioMgr.play("bramble_ult", 0.04, 2.0, center)
	Fx.ring(arena, center, Palette.glow(Color("7ed957"), 1.6), 300.0, 0.7, 8.0)
	Zone.spawn(self, center, {
		"type": "thorn",
		"radius": 300.0,
		"dur": 6.0,
		"dps": 10.0,
	...
	for enemy in arena.heroes_in_circle(center, 300.0):
		if enemy.team == team:
			continue
