...
func _channel_tick(_slot: String, delta: float) -> void:
	if not _brace_started:
		_brace_started = true
		AudioMgr.play("titan_brace", 0.05, 0.0, global_position)
		ability_signature("titan", radius + 55.0)
		controller_rumble(0.08, 0.24, 0.18)
...
func _basic(dir: Vector2) -> void:
	visual.attack_swing()
	AudioMgr.play("titan_punch", 0.08, -4.0, global_position)
	...
	if hits > 0:
		...
		arena.shake(0.1)
		controller_rumble(0.08, 0.16, 0.11)
...
func _aim_begin(_slot: String) -> bool:
	...
	ability_signature("titan", 120.0, aim)
	controller_rumble(0.12, 0.32, 0.18)
...
func _ability1(dir: Vector2) -> void:
	...
	if target == null:
		AudioMgr.play("titan_punch", 0.1, -8.0, global_position)
		visual.attack_swing()
...
func _throw_target(target: Hero, dir: Vector2) -> void:
	...
	AudioMgr.play("titan_throw", 0.05, -1.0, global_position)
	arena.shake(0.25)
	visual.squash(1.25, 0.8)
	ability_signature("titan", 155.0, dir)
	controller_rumble(0.16, 0.5, 0.25)
...
func _aoe_launch() -> void:
	_act("a1")
	AudioMgr.play("titan_launch", 0.04, 1.0, global_position)
	arena.shake(0.4)
	ability_signature("titan", AOE_LAUNCH_RADIUS, aim)
	controller_rumble(0.2, 0.65, 0.34)
...
func _dodge_action(dir: Vector2) -> void:
	...
	AudioMgr.play("dash", 0.1, -5.0, global_position)
	Fx.dust(arena, global_position)
...
func _ultimate(_dir: Vector2) -> void:
	...
	AudioMgr.play("titan_rage", 0.03, 1.0, global_position)
	arena.shake(0.4)
	controller_rumble(0.32, 0.85, 0.6)
