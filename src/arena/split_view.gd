...
##
## Pelaajaottelut käyttävät tätä aina. Bot vs bot -simulaatiossa voidaan käyttää
## klassista GameCameraa, koska siinä ei ole paikallisia pelaajia.
...
const MIN_ZOOM := 0.20            # suuret skillshotit ja 7200x4400-varanäkymä
...
var _pane_count := 1
var _local_groups: Array = []     # paikalliset sankarit eivät vaihdu ottelun aikana
...
## Kutsutaan areenan _ready:stä (kartta ja sankarit ovat valmiina). HUD rakennetaan
## heti, mutta liitetään seuraavalla pääsilmukan kierroksella: Godot ei salli uuden
## lapsen lisäämistä SplitViewiin samalla kun sen olemassa olevat lapset käyvät
## vielä _ready-vaihetta läpi.
func on_arena_ready() -> void:
	var hud := HudLayer.new()
	hud.setup(arena)
	arena.hud = hud
	_attach_hud.call_deferred(hud)
	for cam in _cams:
		cam.make_current()
	_cache_local_groups()
	var cs := _clusters()
	...
	_update_audio_listeners(cs.size())


func _attach_hud(hud: HudLayer) -> void:
	if not is_instance_valid(hud) or hud.get_parent() != null:
		return
	add_child(hud)
	_apply_layout()
...
func _process(delta: float) -> void:
	...
	_update_audio_listeners(count)
	_snap = false


func _update_audio_listeners(count: int) -> void:
	var listeners: Array = []
	for i in range(count):
		listeners.append(_cams[i].global_position)
	AudioMgr.set_listener_positions(listeners)


func _exit_tree() -> void:
	AudioMgr.clear_listener_positions()
	AudioMgr.listener_on = false


## True, jos maailman piste voi näkyä vähintään yhdessä aktiivisessa
## pelaajaruudussa. Pelilogiikka jatkaa aina, vain näkymätön piirto ohitetaan.
func is_position_visible(world_pos: Vector2, margin := 320.0) -> bool:
	for i in range(_pane_count):
		if not _conts[i].visible:
			continue
		var vp_size := Vector2(_vps[i].size)
		if vp_size.x < 1.0 or vp_size.y < 1.0:
			vp_size = _conts[i].size
		if vp_size.x < 1.0 or vp_size.y < 1.0:
			continue
		var zoom: float = maxf(_cams[i].zoom.x, 0.001)
		var half := vp_size / (zoom * 2.0) + Vector2.ONE * margin
		var center: Vector2 = _cams[i].global_position + _cams[i].offset / zoom
		if Rect2(center - half, half * 2.0).has_point(world_pos):
			return true
	return false
...
## Ruutujen suorakulmiot annetulle määrälle. Kahdelle pelaajalle ylä/ala-jako
## säilyttää MOBA-kentän leveänä ja jättää tähtäykselle enemmän sivutilaa.
## Kolme ja neljä pelaajaa käyttävät kompaktia ruudukkoa.
func _pane_rects(count: int, full: Vector2) -> Array:
	...
	match count:
		...
		2:
			return [Rect2(0, 0, full.x, hh), Rect2(0, hh, full.x, full.y - hh)]
		3:
			return [
...
func _clusters() -> Array:
	if _local_groups.is_empty():
		return [_fallback_group()]
	return _local_groups


func _cache_local_groups() -> void:
	_local_groups.clear()
	for hero in arena.heroes:
		if is_instance_valid(hero) and hero.profile != null and not hero.profile.is_bot:
			_local_groups.append([hero])
	_local_groups.sort_custom(func(a, b): return a[0].profile.index < b[0].profile.index)
	if _local_groups.size() > MAX_PANES:
		_local_groups.resize(MAX_PANES)
...
func _update_camera(cam: Camera2D, vp: SubViewport, cluster: Array, delta: float,
	...
	# Etenkin kahden pelaajan leveä ylä/ala-ruutu voi olla karttaa leveämpi.
	# Zoomaa silloin vain sen verran lähemmäs, ettei sivuille jää mustaa tyhjää.
	if arena.map != null:
		var playable_size: Vector2 = arena.map.size() + Vector2.ONE * 96.0
		var fill_zoom: float = maxf(vps.x / playable_size.x, vps.y / playable_size.y)
		tz = clampf(maxf(tz, fill_zoom), MIN_ZOOM, MAX_ZOOM)
